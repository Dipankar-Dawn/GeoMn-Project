...
def get_weather_values(district_data, weather_condition):
    ...
    if weather_condition == "good":
        temperature = float(district_data["Avg_Temperature_C"].quantile(0.25))
        rainfall = float(district_data["Total_Rainfall_mm"].quantile(0.25))
        humidity = float(district_data["Avg_Humidity_pct"].quantile(0.25))
    elif weather_condition == "bad":
        temperature = float(district_data["Avg_Temperature_C"].quantile(0.75))
        rainfall = float(district_data["Total_Rainfall_mm"].quantile(0.75))
        humidity = float(district_data["Avg_Humidity_pct"].quantile(0.75))
    elif weather_condition == "worst":
        temperature = float(district_data["Avg_Temperature_C"].max())
        rainfall = float(district_data["Total_Rainfall_mm"].max())
        humidity = float(district_data["Avg_Humidity_pct"].max())
    else:
...
def calculate_stress(district_data, production, temperature, rainfall, humidity):
    district_avg_production = float(district_data["Production_Tonnes"].mean())
    
    production_stress = (district_avg_production - production) / district_avg_production
    production_stress = float(np.clip(production_stress, 0, 1))

    temp_mean = float(df["Avg_Temperature_C"].mean())
    temp_std = float(df["Avg_Temperature_C"].std())
    temperature_stress = abs(temperature - temp_mean) / (2 * temp_std)
    temperature_stress = float(np.clip(temperature_stress, 0, 1))

    rain_min = float(df["Total_Rainfall_mm"].min())
    rain_max = float(df["Total_Rainfall_mm"].max())
    rainfall_stress = (rainfall - rain_min) / (rain_max - rain_min)
    rainfall_stress = float(np.clip(rainfall_stress, 0, 1))

    humidity_min = float(df["Avg_Humidity_pct"].min())
    humidity_max = float(df["Avg_Humidity_pct"].max())
    humidity_stress = (humidity - humidity_min) / (humidity_max - humidity_min)
    humidity_stress = float(np.clip(humidity_stress, 0, 1))

    weather_stress = (temperature_stress + rainfall_stress + humidity_stress) / 3
...
def get_districts(state: str):
    ...
    districts = sorted(state_data["District"].dropna().unique().tolist())
...
def predict(data: PredictionInput):
    ...
    if district_data.empty:
        raise HTTPException(status_code=404, detail="Selected State/District not found in dataset")
    ...
    temperature, rainfall, humidity = get_weather_values(
        district_data, data.weather_condition
    )

    stress = calculate_stress(
        district_data, production, temperature, rainfall, humidity
    )
    ...
    prediction = shortfall_model.predict(np.array([shortfall_features]))[0]
    ...
    ai_recommendation = generate_gemini_recommendation(
        ...
        weather_stress=round(stress["weather_stress"], 2),
        production_stress=round(stress["production_stress"], 2),
    ...
    if ai_recommendation:
        recommendation_source = "Gemini AI Recommendation"
    else:
        recommendation_source = "GeoMn Rule-Based Recommendation (Offline Fallback)"
